...
{{< figure title="CNCF Project DevStream" src="devstream-landscape.png" >}}

2. [Apache DevLake](https://github.com/apache/incubator-devlake)

{{< figure title="Apache DevLake" src="apache-devlake.png" >}}
...
{{< figure title="DevStream Contributing" src="contributing.png" >}}
...
{{< figure title="Fork DevStream" src="fork.png" >}}

点一下 Fork 按钮，然后回到自己账号下，可以找到 fork 到的项目了：

{{< figure title="DevStream Fork" src="fork-2.png" >}}
...
{{< figure title="DevStream Commit with Goland" src="commit-ide.png" >}}
...
{{< figure title="DevStream Pull Request Template" src="pr-1.png" >}}
...
{{< figure title="DevStream Pull Request 655" src="pr-2.png" >}}
...
{{< figure title="DevStream CI Checks" src="ci.png" >}}
...
{{< figure title="Rebased" src="rebase.png" >}}
...
{{< figure title="Conflict Happened" src="conflict.png" >}}

每次看到这个页面都会让人觉得心头一紧。我们点击“Resolve conflicts”按钮，就可以看到具体冲突的内容了：

{{< figure title="Conflict File" src="conflict-1.png" >}}

可以看到具体冲突的行了，接下来要做的就是解决冲突。我们需要删掉所有的 `<<<<<<<`、`>>>>>>>` 和 `=======` 标记，只保留最终想要的内容，如下：

{{< figure title="Conflict Resolved" src="conflict-2.png" >}}

接着点击右上角的“Mark as Resolved”：

{{< figure title="Mark as resolved" src="conflict-3.png" >}}

最后点击“Commit merge”：

{{< figure title="Commit Merge" src="conflict-4.png" >}}

这样就完成冲突解决了，可以看到产生了一个新的 commit：

{{< figure title="Conflict Resolved" src="conflict-resolved.png" >}}
...
{{< figure title="Conflict Happened" src="conflict-10.png" >}}
...
{{< figure title="Conflict Resolved" src="conflict-resolved-2.png" >}}

可能此时你并不放心，那就通过`git log`命令看一下 commits 历史记录吧：

{{< figure title="Commits History" src="commits-history.png" >}}
...
{{< figure title="Clear Commits Record" src="1commit.png" >}}
...
{{< figure title="Commit Without Sign" src="dco.png" >}}

我不能直接推到 DevStream 项目代码库里演示如何让 DCO 报错，但是如果提 PR，看到的效果是这样的：

{{< figure title="Commit With DCO Error" src="dco-1.png" >}}
